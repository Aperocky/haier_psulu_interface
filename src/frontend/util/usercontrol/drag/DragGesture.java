 ...
 * parent. Also need the parent node in order to correctly position the node in
 * its parent.
 * 
 * @author Feng
 *
...
public class DragGesture {
	...
	/**
	 * Allow the user of this gesture to use the dragged distance in x and y
	 * direction.
	 * 
	 * @param handler
	 *            how to handle the dragged distance
	 */
	...
	public void disable() {
		current.removeEventFilter(MouseEvent.MOUSE_PRESSED, mousePressedHandler(evt -> evt.isSecondaryButtonDown()));
		current.removeEventFilter(MouseEvent.MOUSE_DRAGGED, mouseDraggedHandler(evt -> evt.isSecondaryButtonDown()));
		current.removeEventFilter(MouseEvent.MOUSE_PRESSED, mousePressedHandler(evt -> evt.isPrimaryButtonDown()));
		current.removeEventFilter(MouseEvent.MOUSE_DRAGGED, mouseDraggedHandler(evt -> evt.isPrimaryButtonDown()));
		current.removeEventFilter(MouseEvent.MOUSE_RELEASED, mouseReleasedHandler());
	}
	...
	private EventHandler<MouseEvent> mouseDraggedHandler(Predicate<MouseEvent> mouseTest) {
		return event -> {
			...
			Node node = (Node) event.getSource();
			Point2D anchorInParent = sceneToLocal.apply(new Point2D(event.getSceneX(), event.getSceneY()));
			node.setTranslateX(dragContext.nodeAnchorX + (anchorInParent.getX() - dragContext.mouseAnchorX));
			node.setTranslateY(dragContext.nodeAnchorY + (anchorInParent.getY() - dragContext.mouseAnchorY));
		};
	}
